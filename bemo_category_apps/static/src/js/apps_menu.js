odoo.define("bemo_category_apps.AppsMenuInherit", function (require) {
	"use strict";
		...
		getAppsCategory: function () {
			...
			for (let n in this._apps) {
				if (this._apps[n].ir_ui_menu_category_id) {
					if (
						!category_check_ids.includes(
							this._apps[n].ir_ui_menu_category_id[0],
						)
					) {
						category_check_ids.push(
							this._apps[n].ir_ui_menu_category_id[0],
						);
