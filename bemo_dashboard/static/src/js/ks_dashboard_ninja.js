odoo.define("ks_dashboard_ninja.ks_dashboard", function (require) {
	...
	var KsDashboardNinja = AbstractAction.extend({
		...
		init: function (parent, state, params) {
			...
			this.datetime_format = time.strftime_to_moment_format(
		...
		ks_set_update_interval: function () {
			var self = this;
			if (self.ks_dashboard_data.ks_item_data) {
				Object.keys(self.ks_dashboard_data.ks_item_data).forEach(
					function (item_id) {
						var item_data =
							self.ks_dashboard_data.ks_item_data[item_id];
						...
						if (updateValue) {
							if (!(item_id in self.ksUpdateDashboard)) {
								if (
									...
									var ksItemUpdateInterval = setInterval(
										function () {
								...
								self.ksUpdateDashboard[item_id] =
									ksItemUpdateInterval;
							}
						}
					},
				);
		...
		ks_remove_update_interval: function () {
			...
			if (self.ksUpdateDashboard) {
				Object.values(self.ksUpdateDashboard).forEach(function (
					itemInterval,
				) {
		...
		events: {
			...
			"show.bs.dropdown .ks_dropdown_container":
				"onKsDashboardMenuContainerShow",
			"hide.bs.dropdown .ks_dashboard_item_button_container":
				"onKsDashboardMenuContainerHide",
			...
			"click .ks_dashboard_set_current_layout":
				"_ksSetCurrentLayoutClick",
			"click .ks_dashboard_cancel_current_layout":
				"_ksSetDiscardCurrentLayoutClick",
			"click .ks_add_dashboard_item_on_empty":
				"ks_add_dashboard_item_on_empty",
		},
		...
		ksOnLayoutSelection: function (layout_id) {
			var self = this;
			var selected_layout_name =
				this.ks_dashboard_data.ks_child_boards[layout_id][0];
			var selected_layout_grid_config =
				this.ks_dashboard_data.ks_child_boards[layout_id][1];

			this.gridstackConfig = JSON.parse(selected_layout_grid_config);
			_(this.gridstackConfig).each((x, y) => {
				self.grid.update(
					self.$el.find(".grid-stack-item[data-gs-id=" + y + "]"),
					x["x"],
					x["y"],
					x["width"],
					x["height"],
			...
			this.$el
				.find(
					"#ks_dashboard_layout_dropdown_container .ks_layout_selected",
				)
				.removeClass("ks_layout_selected");
			this.$el
				.find(
					"li.ks_dashboard_layout_event[data-ks_layout_id='" +
						layout_id +
						"']",
				)
				.addClass("ks_layout_selected");
			this.$el
				.find("#ks_dn_layout_button span:first-child")
				.text(selected_layout_name);

			this.$el
				.find(".ks_dashboard_top_menu .ks_dashboard_top_settings")
				.addClass("ks_hide");
			this.$el
				.find(".ks_dashboard_top_menu .ks_am_content_element")
				.addClass("ks_hide");
			this.$el
				.find(".ks_dashboard_layout_edit_mode_settings")
				.removeClass("ks_hide");
		},

		_ksSetCurrentLayoutClick: function () {
			var self = this;
			this.ks_dashboard_data.ks_selected_board_id = this.$el
				.find(
					"#ks_dashboard_layout_dropdown_container .ks_layout_selected",
				)
				.data("ks_layout_id");
			...
			this.ks_dashboard_data.name =
				this.ks_dashboard_data.ks_child_boards[
					this.ks_dashboard_data.ks_selected_board_id
				][0];
			...
			this._rpc({
				...
				args: [
					"update",
					self.ks_dashboard_id,
					{
						ks_selected_board_id:
							this.ks_dashboard_data.ks_selected_board_id,
					},
				],
			});
		...
		ksOnQuickEditView: function (e) {
			...
			var $quickEditButton = $(
				QWeb.render("ksQuickEditButtonContainer", {
					grid: $.extend({}, item_el[0].dataset),
				}),
			);
			$(item_el).before($quickEditButton);

			var ksQuickEditViewWidget = new KsQuickEditView.QuickEditView(
				this,
				{
					item: item_data,
				},
			...
			ksQuickEditViewWidget.appendTo(
				$quickEditButton.find(".dropdown-menu"),
			);
			...
			$quickEditButton.on("hide.bs.dropdown", function (ev) {
				if (
					ev.hasOwnProperty("clickEvent") &&
					document.contains(ev.clickEvent.target)
				) {
					if (ksQuickEditViewWidget) {
					...
					function (notifications) {
						_.each(
							...
							function (notification) {
								if (
									notification[1].hasOwnProperty("type") &&
									notification[1]["type"] ===
										"ks_dashboard_ninja.notification" &&
									self.ks_mode === "active"
								) {
									var item_to_update = _(
										notification[1].changes,
									).filter((x) => {
										return (
											self.ks_dashboard_data.ks_dashboard_items_ids.indexOf(
												x,
											) >= 0
										);
									});
									if (item_to_update) {
										var msg =
											"" +
											item_to_update.length +
											" Dashboard item has been updated.";
										if (item_to_update.length > 1) {
											msg =
												"" +
												item_to_update.length +
												" Dashboard items has been updated.";
										}

										var update_notification_ids = _(
											item_to_update,
										).filter((x) => {
											return (
										...
										if (
											update_notification_ids.length > 0
										) {
											self.call(
												"notification",
												"notify",
										...
										for (
											var i = 0;
											i < update_notification_ids.length;
											i++
										) {
											self.ksFetchUpdateItem(
												update_notification_ids[i],
											);
										}
			...
			return self
				._rpc({
					...
					args: [
						[parseInt(item_id)],
						self.ks_dashboard_id,
						self.ksGetParamsForItemFetch(parseInt(item_id)),
					],
					context: self.getContext(),
				})
				.then(
					function (new_item_data) {
						this.ks_dashboard_data.ks_item_data[item_id] =
							new_item_data[item_id];
		...
		ksRenderChartColorOptions: function (e) {
			var self = this;
			if (
				!$(e.currentTarget).parent().hasClass("ks_date_filter_selected")
			) {
				//            FIXME : Correct this later.
				var $parent = $(e.currentTarget).parent().parent();
				$parent
					.find(".ks_date_filter_selected")
					.removeClass("ks_date_filter_selected");
				$(e.currentTarget).parent().addClass("ks_date_filter_selected");
				var item_data =
					self.ks_dashboard_data.ks_item_data[$parent.data().itemId];
				var chart_data = JSON.parse(item_data.ks_chart_data);
				this.ksChartColors(
					e.currentTarget.dataset.chartColor,
					this.chart_container[$parent.data().itemId],
					$parent.data().chartType,
					$parent.data().chartFamily,
					item_data.ks_bar_chart_stacked,
					item_data.ks_semi_circle_chart,
					item_data.ks_show_data_value,
					chart_data,
					item_data,
				);
				this._rpc({
		...
		normalize_dn_data: function (result) {
			_(result.ks_child_boards).each((x, y) => {
				if (typeof y === "number") {
		...
		ks_fetch_items_data: function () {
			var self = this;
			var items_promises = [];
			self.ks_dashboard_data.ks_dashboard_items_ids.forEach(function (
				item_id,
			) {
				items_promises.push(
					self
						._rpc({
							...
							args: [
								[item_id],
								self.ks_dashboard_id,
								self.ksGetParamsForItemFetch(item_id),
							],
						})
						.then(function (result) {
							self.ks_dashboard_data.ks_item_data[item_id] =
		...
		onKsDashboardMenuContainerShow: function (e) {
			...
			if ($(e.target).hasClass("ks_dashboard_more_action")) {
				...
				var base64_image =
					this.chart_container[chart_id].toBase64Image();
				$(e.target).find(".dropdown-menu").empty();
				$(e.target)
					.find(".dropdown-menu")
					.append(
						$(
							QWeb.render("ksMoreChartOptions", {
								href: base64_image,
		...
		onKsDashboardMenuContainerHide: function (e) {
			...
			if (
				this.ks_dashboard_data.ks_item_data[item_id]["isDrill"] == true
			) {
				clearInterval(this.ksUpdateDashboard[item_id]);
			}
		},
		...
		ksRenderDashboard: function () {
			...
			if (self.ks_dashboard_data.ks_child_boards)
				self.ks_dashboard_data.name =
					this.ks_dashboard_data.ks_child_boards[
						self.ks_dashboard_data.ks_selected_board_id
					][0];
			var $ks_header = $(
				QWeb.render("ksDashboardNinjaHeader", {
					ks_dashboard_name: self.ks_dashboard_data.name,
					ks_dashboard_manager:
						self.ks_dashboard_data.ks_dashboard_manager,
					date_selection_data: self.ks_date_filter_selections,
					date_selection_order: self.ks_date_filter_selection_order,
					ks_show_create_layout_option:
						Object.keys(self.ks_dashboard_data.ks_item_data)
							.length > 0 &&
						self.ks_dashboard_data.ks_dashboard_manager,
					ks_show_layout:
						self.ks_dashboard_data.ks_dashboard_manager &&
					...
					ks_dn_pre_defined_filters: _(
						self.ks_dashboard_data.ks_dashboard_pre_domain_filter,
					)
						.values()
						.sort(function (a, b) {
		...
		ksRenderDashboardMainContent: function () {
			...
			if (self.ks_dashboard_data.ks_item_data) {
				...
				var $dashboard_body_container = $(
					QWeb.render("ks_main_body_container"),
				);
				var $gridstackContainer =
					$dashboard_body_container.find(".grid-stack");
		...
		ksSortItems: function (ks_item_data) {
			...
			if (self.ks_dashboard_data.ks_gridstack_config) {
				self.gridstackConfig = JSON.parse(
				...
				if (self.grid) {
					if (items[i].ks_dashboard_item_type === "ks_tile") {
						...
						if (items[i].id in self.gridstackConfig) {
							self.grid.addWidget(
								$(item_view),
								self.gridstackConfig[items[i].id].x,
								self.gridstackConfig[items[i].id].y,
								self.gridstackConfig[items[i].id].width,
								self.gridstackConfig[items[i].id].height,
								false,
								6,
								null,
								2,
						...
						items[i].ks_dashboard_item_type === "ks_list_view"
					) {
		...
		_ksRenderDashboardTile: function (tile) {
			...
			if (tile.ks_multiplier_active) {
				...
				var data_count = KsGlobalFunction._onKsGlobalFormatter(
					ks_record_count,
					tile.ks_data_formatting,
				);
				var count = field_utils.format.float(
					ks_record_count,
					Float64Array,
				);
			} else {
			...
			if (tile.ks_icon_select == "Custom") {
				if (tile.ks_icon[0]) {
					ks_icon_url =
						"data:image/" +
						(self.file_type_magic_word[tile.ks_icon[0]] || "png") +
						";base64," +
						tile.ks_icon;
				} else {
					ks_icon_url = false;
			...
			tile.ksIsDashboardManager =
				self.ks_dashboard_data.ks_dashboard_manager;
			ks_rgba_background_color = self._ks_get_rgba_format(
				tile.ks_background_color,
			);
			ks_rgba_font_color = self._ks_get_rgba_format(tile.ks_font_color);
			ks_rgba_default_icon_color = self._ks_get_rgba_format(
				tile.ks_default_icon_color,
			);
			ks_rgba_button_color = self._ks_get_rgba_format(
				tile.ks_button_color,
			);
			style_main_body =
				"background-color:" +
			...
			switch (tile.ks_layout) {
				case "layout1":
					item_view = QWeb.render("ks_dashboard_item_layout1", {
						...
						ks_dashboard_list:
							self.ks_dashboard_data.ks_dashboard_list,
				...
				case "layout2":
					var ks_rgba_dark_background_color_l2 =
						self._ks_get_rgba_format(
							self.ks_get_dark_color(
					...
					style_image_body_l2 =
						"background-color:" +
						ks_rgba_dark_background_color_l2 +
						";";
				...
				default:
					item_view = QWeb.render(
						"ks_dashboard_item_layout_default",
						{
							item: tile,
						},
					);
		...
		_renderGraph: function (item) {
			...
			var $ks_gridstack_container = $(
				QWeb.render("ks_gridstack_container", {
					ks_chart_title: chart_title,
					ksIsDashboardManager:
						self.ks_dashboard_data.ks_dashboard_manager,
			...
			if (chart_id in self.gridstackConfig) {
				self.grid.addWidget(
					$ks_gridstack_container,
					self.gridstackConfig[chart_id].x,
					self.gridstackConfig[chart_id].y,
					self.gridstackConfig[chart_id].width,
					self.gridstackConfig[chart_id].height,
					false,
					11,
					null,
					3,
		...
		_renderChart: function ($ks_gridstack_container, item) {
			...
			if (item.ks_chart_cumulative_field) {
				for (var i = 0; i < chart_data.datasets.length; i++) {
					...
					if (chart_data.datasets[i].ks_chart_cumulative_field) {
						for (
							var j = 0;
							j < chart_data.datasets[i].data.length;
							j++
						) {
							ks_temp_com =
								ks_temp_com + chart_data.datasets[i].data[j];
							data.push(ks_temp_com);
						}
						datasets.label =
							"Cumulative" + chart_data.datasets[i].label;
						datasets.data = data;
			...
			var $ksChartContainer = $(
				'<canvas id="ks_chart_canvas_id" data-chart-id=' +
					chart_id +
					"/>",
			);
			$ks_gridstack_container
				.find(".card-body")
				.append($ksChartContainer);
			if (!item.ks_show_records) {
				$ks_gridstack_container
					.find(".ks_dashboard_item_chart_info")
					.hide();
			}
			...
			if (chart_family === "circle") {
				if (chart_data && chart_data["labels"].length > 30) {
					$ks_gridstack_container
						.find(".ks_dashboard_color_option")
						.remove();
					$ks_gridstack_container
						.find(".card-body")
						.empty()
						.append(
							$(
								"<div style='font-size:20px;'>Too many records for selected Chart Type. Consider using <strong>Domain</strong> to filter records or <strong>Record Limit</strong> to limit the no of records under <strong>30.</strong>",
							),
						);
					return;
				}
			...
			if (
				chart_data["ks_show_second_y_scale"] &&
						...
						ticks: {
							...
							callback: function (value, index, values) {
								...
								if (ks_selection === "monetary") {
									...
									var ks_data = value;
									ks_data =
										KsGlobalFunction._onKsGlobalFormatter(
											ks_data,
											item.ks_data_formatting,
										);
									ks_data = KsGlobalFunction.ks_monetary(
										ks_data,
										ks_currency_id,
									);
									return ks_data;
									...
									return KsGlobalFunction._onKsGlobalFormatter(
										value,
										item.ks_data_formatting,
									);
								}
			...
			if (chart_data && chart_data["datasets"].length > 0)
				self.ksChartColors(
					item.ks_chart_item_color,
					ksMyChart,
					chart_type,
					chart_family,
					item.ks_bar_chart_stacked,
					item.ks_semi_circle_chart,
					...
					for (var j = 0; j < gradientKeys.length; j++) {
						...
						if (gradientIndex === +gradientKey) {
							...
							var gradientPartIndex =
								(gradientIndex - prevKey) /
								(gradientKey - prevKey); //Calculate where
							var color = [];
							for (var k = 0; k < 4; k++) {
			...
			if (ksChartFamily == "circle") {
				...
				options.tooltips.callbacks = {
					title: function (tooltipItem, data) {
						...
						if (ks_selection === "monetary") {
							var ks_currency_id = chart_data.ks_currency;
							k_amount = KsGlobalFunction.ks_monetary(
								k_amount,
								ks_currency_id,
							);
							return (
							...
							k_amount = field_utils.format.float(
								k_amount,
								Float64Array,
							);
							return (
				...
				options.plugins.datalabels.formatter = function (value, ctx) {
					...
					if (ks_selection === "monetary") {
						var ks_currency_id = chart_data.ks_currency;
						var ks_data = value;
						ks_data = KsGlobalFunction._onKsGlobalFormatter(
							ks_data,
							item.ks_data_formatting,
						);
						ks_data = KsGlobalFunction.ks_monetary(
				...
				if (chartType === "line") {
					options.plugins.datalabels.backgroundColor = function (
						context,
					) {
				...
				if (chartType === "horizontalBar") {
					options.scales.xAxes[0].ticks.callback = function (
						value,
						index,
						values,
					) {
		...
		onChartCanvasClick: function (evt) {
			...
			if (evt.currentTarget.classList.value !== "ks_list_canvas_click") {
				...
				if (activePoint) {
					var item_data =
						self.ks_dashboard_data.ks_item_data[item_id];
					var groupBy = JSON.parse(item_data["ks_chart_data"])[
						"groupby"
					];
					if (activePoint._chart.data.domains) {
						...
						var domain =
							activePoint._chart.data.domains[activePoint._index];
						if (
							item_data.max_sequnce != 0 &&
							sequnce < item_data.max_sequnce
						) {
							self._rpc({
								model: "ks_dashboard_ninja.item",
								...
								if (result.ks_chart_data) {
									self.ks_dashboard_data.ks_item_data[
										...
										result.ks_chart_type;
									self.ks_dashboard_data.ks_item_data[
										item_id
									]["ks_chart_data"] = result.ks_chart_data;
									if (
										self.ks_dashboard_data.ks_item_data[
											item_id
										].domains
										...
										self.ks_dashboard_data.ks_item_data[
											item_id
										]["domains"][result.sequence] =
											JSON.parse(
												result.ks_chart_data,
											).previous_domain;
									} else {
										self.ks_dashboard_data.ks_item_data[
											item_id
									...
									$(
										self.$el
											.find(
												".grid-stack-item[data-gs-id=" +
													item_id +
													"]",
											)
											.children()[0],
									)
										.find(".ks_dashboard_item_drill_up")
										.removeClass("d-none");
									$(
										self.$el
											.find(
									...
									var $container =
										self.renderListViewData(item_data);
									$(
										self.$el
											.find(
												".grid-stack-item[data-gs-id=" +
							...
							if (item_data.action) {
								if (!item_data.ks_is_client_action) {
									...
									if (action.view_mode.includes("tree"))
										action.view_mode =
											action.view_mode.replace(
												"tree",
												"list",
											);
									for (
										var i = 0;
										i < action.views.length;
										i++
									)
										action.views[i][1].includes("tree")
											? (action.views[i][1] =
													action.views[i][1].replace(
														"tree",
														"list",
													))
											: action.views[i][1];
						...
						self._rpc({
							...
							if (result.ks_list_view_data) {
								if (
									...
									self.ks_dashboard_data.ks_item_data[
										item_id
									]["domains"][result.sequence] = JSON.parse(
										result.ks_list_view_data,
									).previous_domain;
								} else {
									self.ks_dashboard_data.ks_item_data[
								...
								$(
									...
									.find(".ks_dashboard_item_action_export")
									.addClass("d-none");
								$(
									self.$el
										.find(
		...
		ksOnDrillUp: function (e) {
			...
			if (item_data) {
				if ("domains" in item_data) {
					domain = item_data["domains"][item_data.sequnce - 1]
						? item_data["domains"][item_data.sequnce - 1]
						: [];
					...
					if (sequnce >= 0) {
						...
						$(
							...
							.find(".ks_dashboard_quick_edit_action_popup")
							.addClass("d-sm-block ");
						$(
							self.$el
								.find(
			...
			return self
				._rpc({
					...
					args: [
						[item_data.id],
						self.ks_dashboard_id,
						self.ksGetParamsForItemFetch(id),
					],
					context: self.getContext(),
				})
				.then(
					function (new_item_data) {
						...
						$(
							self.$el
								.find(".grid-stack-item[data-gs-id=" + id + "]")
								.children()[0],
						)
							.find(".card-body")
							.empty();
						...
						if (item_data.ks_list_view_data) {
							...
							var ks_length = JSON.parse(
								item_data["ks_list_view_data"],
							).data_rows.length;
							if (
								new_item_data["ks_list_view_type"] ===
									"ungrouped" &&
								JSON.parse(item_data["ks_list_view_data"])
									.data_rows.length
							) {
								item_view
									.find(".ks_pager")
									.removeClass("d-none");
								if (
									item.ks_record_count <=
									item.ks_pagination_limit
								)
									item_view
										.find(".ks_load_next")
		...
		onChartMoreInfoClick: function (evt) {
			...
			var item_data = self.ks_dashboard_data.ks_item_data[item_id];
			var groupBy =
				item_data.ks_chart_groupby_type === "relational_type"
					? item_data.ks_chart_relation_groupby_name
					: item_data.ks_chart_relation_groupby_name +
					  ":" +
					  item_data.ks_chart_date_groupby;
			...
			if (item_data.ks_show_records) {
				if (item_data.action) {
					if (!item_data.ks_is_client_action) {
						...
						if (action.view_mode.includes("tree"))
							action.view_mode = action.view_mode.replace(
								"tree",
								"list",
							);
						for (var i = 0; i < action.views.length; i++)
							action.views[i][1].includes("tree")
								? (action.views[i][1] = action.views[
										i
								  ][1].replace("tree", "list"))
								: action.views[i][1];
		...
		_ksRenderEditMode: function () {
			...
			self.$el
				.find(".ks_item_click")
				.addClass("ks_item_not_click")
				.removeClass("ks_item_click");
			self.$el
				.find(".ks_dashboard_item")
				.removeClass("ks_dashboard_item_header_hover");
			self.$el
				.find(".ks_dashboard_item_header")
				.removeClass("ks_dashboard_item_header_hover");

			self.$el
				.find(".ks_dashboard_item_l2")
				.removeClass("ks_dashboard_item_header_hover");
			self.$el
				.find(".ks_dashboard_item_header_l2")
				.removeClass("ks_dashboard_item_header_hover");

			self.$el
				.find(".ks_dashboard_item_l5")
				.removeClass("ks_dashboard_item_header_hover");

			self.$el
				.find(".ks_dashboard_item_button_container")
				.removeClass("ks_dashboard_item_header_hover");

			self.$el.find(".ks_dashboard_link").addClass("ks_hide");
			self.$el.find(".ks_dashboard_top_settings").addClass("ks_hide");
			self.$el
				.find(".ks_dashboard_edit_mode_settings")
				.removeClass("ks_hide");
		...
		_ksRenderActiveMode: function () {
			...
			if (self.ks_dashboard_data.ks_child_boards) {
				var $layout_container = $(
					QWeb.render("ks_dn_layout_container", {
						ks_selected_board_id:
							self.ks_dashboard_data.ks_selected_board_id,
						ks_child_boards: self.ks_dashboard_data.ks_child_boards,
					}),
				);
				$("#ks_dashboard_title .ks_am_element").replaceWith(
					$layout_container,
				);
				$("#ks_dashboard_title_label").replaceWith($layout_container);
			} else {
				$("#ks_dashboard_title_label").text(
					self.ks_dashboard_data.name,
				);
			}
			...
			if (self.ks_dashboard_data.ks_item_data)
				$(".ks_am_content_element").removeClass("ks_hide");

			self.$el
				.find(".ks_item_not_click")
				.addClass("ks_item_click")
				.removeClass("ks_item_not_click");
			self.$el
				.find(".ks_dashboard_item")
				.addClass("ks_dashboard_item_header_hover");
			self.$el
				.find(".ks_dashboard_item_header")
				.addClass("ks_dashboard_item_header_hover");
			...
			self.$el
				.find(".ks_chart_container")
				.removeClass("ks_item_not_click ks_item_click");
			self.$el
				.find(".ks_list_view_container")
				.removeClass("ks_item_click");
				...
				self.do_action(
					{
						...
						context: {
							...
							ks_set_interval:
								self.ks_dashboard_data.ks_set_interval,
							ks_data_formatting:
								self.ks_dashboard_data.ks_data_formatting,
						},
					},
					{
		...
		ksImportItem: function (e) {
			...
			fileReader.readAsText(
				$(".ks_input_import_item_button").prop("files")[0],
			);
		},
		...
		_onKsSaveLayoutClick: function () {
			...
			if (
				...
				dashboard_title != 0 &&
				dashboard_title !== self.ks_dashboard_data.name
			) {
		...
		_onKsCreateLayoutClick: function () {
			...
			if (dashboard_title === "") {
				self.call("notification", "notify", {
					message:
						"Dashboard Name is required to save as New Layout.",
					type: "warning",
				});
			} else {
				if (!self.ks_dashboard_data.ks_child_boards) {
				...
				this._rpc({
					...
					self.ks_update_child_board_value(
						dashboard_title,
						res_id,
						grid_config,
					),
		...
		ks_update_child_board_value: function (
			dashboard_title,
			res_id,
			grid_config,
		) {
			...
			var update_data = {};
			update_data[child_board_id] = [
				dashboard_title,
				JSON.stringify(grid_config),
			];
			self.ks_dashboard_data.ks_child_boards = _.extend(
				update_data,
				self.ks_dashboard_data.ks_child_boards,
			);
		},
			...
			if (self.ksAllowItemClick) {
				...
				if (e.target.title != "Customize Item") {
					var item_id = parseInt(
						e.currentTarget.firstElementChild.id,
					);
					var item_data =
						self.ks_dashboard_data.ks_item_data[item_id];
		...
		_onKsItemCustomizeClick: function (e) {
			...
			var id = parseInt(
				$(
					$(e.currentTarget).parentsUntil(".grid-stack").slice(-1)[0],
				).attr("data-gs-id"),
			);
		...
		ksUpdateDashboardItem: function (ids) {
			...
			for (var i = 0; i < ids.length; i++) {
				...
				if (item_data["ks_dashboard_item_type"] == "ks_list_view") {
					var item_view = self.$el.find(
						".grid-stack-item[data-gs-id=" + item_data.id + "]",
					);
					var name = item_data.name
						? item_data.name
						: item_data.ks_model_display_name;
					item_view
						.children()
						.find(".ks_list_view_heading")
						.prop("title", name);
					item_view
						.children()
						.find(".ks_list_view_heading")
					...
					item_view
						.find(".card-body")
						.append(self.renderListViewData(item_data));
					var rows = JSON.parse(
						item_data["ks_list_view_data"],
					).data_rows;
					...
					if (ks_length) {
						if (item_view.find(".ks_pager_name")) {
							...
							var $ks_pager_container = QWeb.render(
								"ks_pager_template",
								{
									item_id: ids[i],
							...
							item_view
								.find(".ks_pager_name")
								.append($($ks_pager_container));
						}

						if (ks_length < item_data.ks_pagination_limit)
							item_view
								.find(".ks_load_next")
								.addClass("ks_event_offer_list");
						item_view
						...
						if (
							item_data.ks_record_data_limit ==
								item_data.ks_pagination_limit ||
							item_data.ks_record_count ==
								item_data.ks_pagination_limit
						) {
							item_view
								.find(".ks_load_next")
					...
					self.$el
						...
						.find(".ks_dashboard_item_hover")
						.replaceWith($(item_view).find(".ks_dashboarditem_id"));
		...
		ks_delete_item: function (id, item) {
			...
			Dialog.confirm(
				this,
				_t("Are you sure you want to remove this item?"),
				{
					confirm_callback: function () {
						self._rpc({
							...
							$.when(self.ks_fetch_data()).then(function () {
								$.when(self.ks_fetch_items_data()).then(
									function () {
										self.ks_remove_update_interval();
										self.ksRenderDashboard();
										self.ks_set_update_interval();
		...
		_ksSaveCurrentLayout: function () {
			...
			if (
				...
				if (
					this.ks_dashboard_data.ks_selected_board_id !== "ks_default"
				) {
					model = "ks_dashboard_ninja.child_board";
					rec_id = this.ks_dashboard_data.ks_selected_board_id;
				}
		...
		ks_get_current_gridstack_config: function () {
			...
			if (self.ks_dashboard_data.ks_gridstack_config) {
				_.extend(
					grid_config,
					JSON.parse(self.ks_dashboard_data.ks_gridstack_config),
				);
			}
			if (items) {
				for (var i = 0; i < items.length; i++) {
		...
		_renderListView: function (item, grid) {
			...
			var $ks_gridstack_container = $(
				QWeb.render("ks_gridstack_list_view_container", {
			...
			if (
				...
				item.ks_list_view_type === "ungrouped"
			) {
				$ks_gridstack_container
			...
			if (item_id in self.gridstackConfig) {
				grid.addWidget(
					$ks_gridstack_container,
					self.gridstackConfig[item_id].x,
					self.gridstackConfig[item_id].y,
					self.gridstackConfig[item_id].width,
					self.gridstackConfig[item_id].height,
					false,
					12,
					null,
					3,
		...
		renderListViewData: function (item) {
			...
			if (item.ks_list_view_type === "ungrouped" && list_view_data) {
				if (list_view_data.date_index) {
					...
					for (var i = 0; i < index_data.length; i++) {
						for (
							var j = 0;
							j < list_view_data.data_rows.length;
							j++
						) {
							...
							if (date) {
								if (
									list_view_data.fields_type[index] === "date"
								) {
									list_view_data.data_rows[j]["data"][index] =
										field_utils.format.date(
											moment(moment(date).utc(true)._d),
											{},
											{
												timezone: false,
											},
			...
			if (list_view_data) {
				for (var i = 0; i < list_view_data.data_rows.length; i++) {
					for (
						var j = 0;
						j < list_view_data.data_rows[0]["data"].length;
						j++
					) {
						if (
							typeof list_view_data.data_rows[i].data[j] ===
								"number" ||
			...
			var $ksItemContainer = $(
				QWeb.render("ks_list_view_table", {
					list_view_data: list_view_data,
					item_id: item_id,
					list_type: item.ks_list_view_type,
					isDrill:
						self.ks_dashboard_data.ks_item_data[item_id]["isDrill"],
				}),
			);
		...
		ksListViewBody: function (list_view_data, item_id) {
			...
			var ks_data_calculation_type =
				self.ks_dashboard_data.ks_item_data[item_id]
					.ks_data_calculation_type;
			var list_view_type =
				self.ks_dashboard_data.ks_item_data[item_id].ks_list_view_type;
			var $ksitemBody = $(
				QWeb.render("ks_list_view_tmpl", {
					list_view_data: list_view_data,
					item_id: itemid,
					calculation_type: ks_data_calculation_type,
					isDrill:
		...
		ksSum: function (
			...
			if (field.ks_multiplier_active) {
				item_info["count"] = KsGlobalFunction._onKsGlobalFormatter(
					count * field.ks_multiplier,
					field.ks_data_formatting,
				);
				item_info["count_tooltip"] = field_utils.format.float(
					count * field.ks_multiplier,
					Float64Array,
				);
			} else {
			...
			if (target_deviation !== Infinity)
				item_info.target_deviation =
					field_utils.format.integer(target_deviation) + "%";
			else {
			...
			item_info.target_progress_deviation =
				field_utils.format.integer(target_progress_deviation) + "%";
		...
		ksPercentage: function (
			...
			item_info["count"] = count
				? field_utils.format.integer(count) + "%"
				: "0%";
		...
		renderKpi: function (item, grid) {
			...
			if (field.ks_date_filter_selection === "l_none")
				ks_date_filter_selection =
					self.ks_dashboard_data.ks_date_filter_selection;
			...
			var ks_rgba_background_color = self._ks_get_rgba_format(
				field.ks_background_color,
			);
			var ks_rgba_button_color = self._ks_get_rgba_format(
				field.ks_button_color,
			);
			var ks_rgba_font_color = self._ks_get_rgba_format(
				field.ks_font_color,
			);
			if (field.ks_goal_enable) {
				...
				if (deviation !== Infinity)
					deviation = deviation
						? field_utils.format.integer(deviation) + "%"
						: 0 + "%";
			}
			if (
				...
				var pre_deviation = previous_period_data
					? field_utils.format.integer(
							parseInt(
								(pre_diffrence / previous_period_data) * 100,
							),
					  ) + "%"
					: "100%";
			}
			item["ksIsDashboardManager"] =
				self.ks_dashboard_data.ks_dashboard_manager;
			var ks_icon_url;
			if (field.ks_icon_select == "Custom") {
				if (field.ks_icon[0]) {
					ks_icon_url =
						"data:image/" +
						(self.file_type_magic_word[field.ks_icon[0]] || "png") +
						";base64," +
						field.ks_icon;
				} else {
					ks_icon_url = false;
			...
			if (field.ks_multiplier_active) {
				var target_progress_deviation = String(
					Math.round(
						((count_1 * field.ks_multiplier) / target_1) * 100,
					),
				);
			}
			var ks_rgba_icon_color = self._ks_get_rgba_format(
				field.ks_default_icon_color,
			);
			var item_info = {
				item: item,
				id: field.id,
				count_1: KsGlobalFunction.ksNumFormatter(
					kpi_data[0]["record_data"],
					1,
				),
				count_1_tooltip: kpi_data[0]["record_data"],
				count_2: kpi_data[1]
					? String(kpi_data[1]["record_data"])
					: false,
				name: field.name
					? field.name
					: field.ks_model_id.data.display_name,
				...
				ks_previous_period:
					ks_valid_date_selection.indexOf(ks_date_filter_selection) >=
					0
						? field.ks_previous_period
						: false,
			...
			if (item_info.target_deviation === Infinity)
				item_info.target_arrow = false;
			if (field.ks_multiplier_active) {
				item_info["count_1"] = KsGlobalFunction._onKsGlobalFormatter(
			...
			item_info["target"] = KsGlobalFunction._onKsGlobalFormatter(
				kpi_data[0].target,
				field.ks_data_formatting,
			);
			var $kpi_preview;
			if (!kpi_data[1]) {
				if (
					...
					$kpi_preview = $(
						QWeb.render("ks_kpi_template_3", item_info),
					);
					$kpi_preview
						.find("#ks_progressbar")
				...
				if (
					$kpi_preview.find(".ks_target_previous").children()
						.length !== 2
				) {
					$kpi_preview
				...
				switch (field.ks_data_comparison) {
					case "None":
						if (field.ks_multiplier_active) {
							...
							var count =
								String(
									KsGlobalFunction.ksNumFormatter(
							...
							var count_tooltip =
								String(count_1) + "/" + String(count_2);
							var count =
								String(
									KsGlobalFunction.ksNumFormatter(count_1, 1),
								) +
								"/" +
								String(
									KsGlobalFunction.ksNumFormatter(count_2, 1),
								);
							item_info["count"] =
								String(
									KsGlobalFunction._onKsGlobalFormatter(
						...
						$kpi_preview = $(
							QWeb.render("ks_kpi_template_2", item_info),
						);
						break;
					case "Sum":
						$kpi_preview = self.ksSum(
							count_1,
							count_2,
							item_info,
							field,
					...
					case "Percentage":
						...
						break;
					case "Ratio":
						var gcd = self.ks_get_gcd(
							Math.round(count_1),
							Math.round(count_2),
						);
						if (item.ks_data_formatting == "exact") {
							if (count_1 && count_2) {
								item_info["count_tooltip"] =
									count_1 / gcd + ":" + count_2 / gcd;
								item_info["count"] =
									count_1 / gcd + ":" + count_2 / gcd;
							} else {
								item_info["count_tooltip"] =
		...
		onKsDuplicateItemClick: function (e) {
			var self = this;
			var ks_item_id = $(
				$(e.target).parentsUntil(".ks_dashboarditem_id").slice(-1)[0],
			)
				.parent()
				.attr("id");
			var dashboard_id = $(
				...
				.find(".ks_dashboard_select")
				.val();
			var dashboard_name = $(
				$(e.target).parentsUntil(".ks_dashboarditem_id").slice(-1)[0],
			)
				.find(".ks_dashboard_select option:selected")
				.text();
			this._rpc({
				model: "ks_dashboard_ninja.item",
				method: "copy",
				...
				self.do_notify(
					_t("Item Duplicated"),
					_t(
						"Selected item is duplicated to " +
							dashboard_name +
							" .",
					),
				);
		...
		ksOnListItemInfoClick: function (e) {
			...
			if (e.currentTarget.dataset.listViewType === "ungrouped") {
				...
				if (e.currentTarget.dataset.listType === "date_type") {
					...
					e.currentTarget.dataset.listType === "relational_type"
				) {
					var domain = JSON.parse(
						e.currentTarget.parentElement.parentElement.dataset
							.domain,
					...
					action["domain"] = domain;
					action["context"][
						"search_default_" + e.currentTarget.dataset.groupby
					] = parseInt(e.currentTarget.dataset.recordId);
		...
		_KsGetDateValues: function () {
			...
			self.$el
				.find("#" + date_filter_selected)
				.addClass("ks_date_filter_selected");
			self.$el
				.find("#ks_date_filter_selection")
				.text(self.ks_date_filter_selections[date_filter_selected]);

			if (
				self.ks_dashboard_data.ks_date_filter_selection === "l_custom"
			) {
		...
		_onKsClearDateValues: function (ks_l_none = false) {
			...
			self.ks_fetch_items_data().then(function () {
				self.ksRenderDashboard();
				$(".ks_date_input_fields").addClass("ks_hide");
				$(".ks_date_filter_dropdown").removeClass(
		...
		_renderDateFilterDatePicker: function () {
			var self = this;
			self.$el.find(".ks_dashboard_link").removeClass("ks_hide");
			var startDate = self.ks_dashboard_data.ks_dashboard_start_date
				? moment
						.utc(self.ks_dashboard_data.ks_dashboard_start_date)
						.local()
				: moment();
			var endDate = self.ks_dashboard_data.ks_dashboard_end_date
				? moment
						.utc(self.ks_dashboard_data.ks_dashboard_end_date)
						.local()
				: moment();

			this.ksStartDatePickerWidget = new datepicker.DateTimeWidget(this);

			this.ksStartDatePickerWidget
				.appendTo(self.$el.find(".ks_date_input_fields"))
				.then(
					function () {
						this.ksStartDatePickerWidget.$el.addClass(
							"ks_btn_middle_child o_input",
						);
						this.ksStartDatePickerWidget.$el
							.find("input")
							.attr("placeholder", "Start Date");
						this.ksStartDatePickerWidget.setValue(startDate);
						this.ksStartDatePickerWidget.on(
							"datetime_changed",
							this,
							function () {
								self.$el
									.find(".apply-dashboard-date-filter")
									.removeClass("ks_hide");
								self.$el
									.find(".clear-dashboard-date-filter")
									.removeClass("ks_hide");
							},
						);
					}.bind(this),
			...
			this.ksEndDatePickerWidget
				.appendTo(self.$el.find(".ks_date_input_fields"))
				.then(
					function () {
						this.ksEndDatePickerWidget.$el.addClass(
							"ks_btn_last_child o_input",
						);
						this.ksStartDatePickerWidget.$el
							.find("input")
							.attr("placeholder", "Start Date");
						this.ksEndDatePickerWidget.setValue(endDate);
						this.ksEndDatePickerWidget.on(
							"datetime_changed",
							this,
							function () {
								self.$el
		...
		_onKsApplyDateFilter: function (e) {
			...
			if (start_date === "Invalid date") {
				...
				self.$el.find(".ks_date_filter_selected").attr("id") !==
				"l_custom"
			) {
				self.ksDateFilterSelection = self.$el
					.find(".ks_date_filter_selected")
					.attr("id");
				...
				if (start_date && end_date) {
					if (
						moment(start_date, self.datetime_format) <=
						moment(end_date, self.datetime_format)
					) {
			...
			if (e.target.id !== "ks_date_selector_container") {
				...
				_.each(
					...
					function ($filter_options) {
						$($filter_options).removeClass(
							"ks_date_filter_selected",
						);
					},
				);
				$(e.target.parentElement).addClass("ks_date_filter_selected");
				$("#ks_date_filter_selection").text(
					self.ks_date_filter_selections[e.target.parentElement.id],
				);

				if (e.target.parentElement.id !== "l_custom") {
					e.target.parentElement.id === "l_none"
					...
					$(".ks_date_filter_dropdown").addClass(
						"ks_btn_first_child_radius",
					);
					self.$el
						.find(".apply-dashboard-date-filter")
		...
		ksChartExportXlsCsv: function (e) {
			...
			if (
				this.ks_dashboard_data.ks_item_data[chart_id]
		...
		ksChartExportPdf: function (e) {
			...
			var $ks_el = $(
				$(
					self.$el.find(
						".grid-stack-item[data-gs-id=" + chart_id + "]",
					),
		...
		ksLoadMoreRecords: function (e) {
			...
			var offset =
				self.ks_dashboard_data.ks_item_data[itemId].ks_pagination_limit;
			...
			this._rpc({
				...
				self.ks_dashboard_data.ks_item_data[itemId][
					"ks_list_view_data"
				] = result.ks_list_view_data;
				var item_view = self.$el.find(
					".grid-stack-item[data-gs-id=" + item_data.id + "]",
				...
				$(e.currentTarget)
					.parents(".ks_pager")
					.find(".ks_value")
					.text(result.offset + "-" + result.next_offset);
				...
				$(e.currentTarget.parentElement)
					.find(".ks_load_previous")
					.removeClass("ks_event_offer_list");
				if (
					result.next_offset <
						parseInt(result.offset) + (offset - 1) ||
		...
		ksLoadPreviousRecords: function (e) {
			...
			var offset =
				self.ks_dashboard_data.ks_item_data[itemId].ks_pagination_limit;
			var ks_offset =
				parseInt(e.target.parentElement.dataset.prevOffset) -
				(offset + 1);
