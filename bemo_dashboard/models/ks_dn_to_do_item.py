	...
	def _ksGetToDOData(self):
		...
		if self.ks_dn_header_lines:
			for ks_dn_header_line in self.ks_dn_header_lines:
				ks_to_do_data['label'].append(ks_dn_header_line.ks_to_do_header)
				...
				ks_dn_header_line.ks_to_do_header = ks_dn_header_line.ks_to_do_header.replace(" ", "")
				if ks_dn_header_line.ks_to_do_header[0].isdigit():
					ks_dn_header_line.ks_to_do_header = ks_dn_header_line.ks_to_do_header.replace(
