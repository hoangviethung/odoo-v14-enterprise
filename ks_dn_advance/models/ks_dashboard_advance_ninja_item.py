...
class KsDashboardNinjaItemAdvance(models.Model):
	...
	ks_custom_query = fields.Text(string="Custom Query", help=' Fetch, combine, and compare data by generating SQL query on your own.')
	ks_data_calculation_type = fields.Selection([('custom', 'Default Query'),
												 ('query', 'Custom Query')], string="Data Calculation Type", default="custom")
	...
	ks_is_date_ranges = fields.Boolean('Date Ranges', help=' Checkbox to apply default date range filter. The date filter applied will also reflect on the main page.')
			...
			with api.Environment.manage():
				if rec.ks_data_calculation_type == 'query' and rec.ks_dashboard_item_type not in ['ks_tile','ks_kpi']\
					...
					if rec.ks_dashboard_item_type == 'ks_list_view':
					   rec.ks_query_result = rec.ks_get_list_query_result(ks_query, selected_start_date, selected_end_date)
					else:
						try:
							type_code = []
							...
							if rec.ks_is_date_ranges:
								...
								new_env.cr.execute("with ks_chart_query as (" + ks_query + ")" +
	...
	def ks_get_list_query_result(self,ks_query, selected_start_date, selected_end_date, ks_offset=0, ks_export_all=False):
		try:
			type_code = []
			conn = sql_db.db_connect(self.env.cr.dbname)
			new_env = api.Environment(conn.cursor(), self.env.uid,
			...
			if self.ks_is_date_ranges:
				...
				new_env.cr.execute("with ks_list_query as (" + ks_query + ")" + "select * from ks_list_query limit "
