odoo.define(
	"ks_dashboard_ninja_list.ks_image_basic_widget",
	function (require) {
		"use strict";
		...
		var KsImageWidget = basic_fields.FieldBinaryImage.extend({
			...
			events: _.extend(
				{},
				basic_fields.FieldBinaryImage.prototype.events,
				{
					"click .ks_icon_container_list": "ks_icon_container_list",
					"click .ks_image_widget_icon_container":
						"ks_image_widget_icon_container",
					"click .ks_icon_container_open_button":
			...
			_render: function () {
				...
				if (ks_self.value) {
					...
					ks_self.$("> span").remove();
					$("<span>")
						.addClass("fa fa-" + ks_self.recordData.ks_default_icon)
						.appendTo(ks_self.$el)
						.css("color", "black");
				} else {
					var $img = $(
				...
				var $ks_icon_container_modal = $(
					QWeb.render("ks_icon_container_modal_template", {
			...
			ks_icon_container_list: function (e) {
				var self = this;
				self.ksSelectedIcon = $(e.currentTarget)
					.find("span")
					.attr("id")
					.split(".")[1];
			...
			ks_fa_icon_search: function (e) {
				...
				var ks_fa_icon_name = self.$el.find(".ks_modal_icon_input")[0]
					.value;
				if (ks_fa_icon_name.slice(0, 3) === "fa-") {
					ks_fa_icon_name = ks_fa_icon_name.slice(3);
				}
				var ks_fa_icon_render = $("<div>").addClass(
					"ks_icon_container_list ks_fa_search_icon",
				);
				$("<span>")
					.attr("id", "ks." + ks_fa_icon_name.toLocaleLowerCase())
					.addClass(
						"fa fa-" +
							ks_fa_icon_name.toLocaleLowerCase() +
							" fa-4x",
					)
					.appendTo($(ks_fa_icon_render));
				$(ks_fa_icon_render).appendTo(
					self.$el.find(".ks_icon_container_grid_view"),
				);
			},
