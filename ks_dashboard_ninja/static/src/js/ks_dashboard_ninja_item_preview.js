odoo.define(
	"ks_dashboard_ninja_list.ks_dashboard_item_preview",
	function (require) {
		"use strict";
		...
		var KsItemPreview = AbstractField.extend({
			...
			_render: function () {
				...
				var ks_rgba_background_color,
					...
					ks_rgba_icon_color;
				self.$el.empty();
				ks_rgba_background_color = self._get_rgba_format(
					field.ks_background_color,
				);
				ks_rgba_font_color = self._get_rgba_format(field.ks_font_color);
				ks_rgba_icon_color = self._get_rgba_format(
				...
				item_info = {
					...
					count_tooltip: field_utils.format.float(
						field.ks_record_count,
						Float64Array,
					),
				};
				...
				if (field.ks_icon) {
					if (!utils.is_bin_size(field.ks_icon)) {
						...
						item_info["img_src"] =
							"data:image/" +
							(self.file_type_magic_word[field.ks_icon[0]] ||
								"png") +
							";base64," +
							field.ks_icon;
					} else {
				...
				if (field.ks_multiplier_active) {
					var ks_record_count =
						field.ks_record_count * field.ks_multiplier;
					item_info["count"] = self._onKsGlobalFormatter(
						ks_record_count,
						field.ks_data_format,
					);
				...
				switch (field.ks_layout) {
					case "layout1":
						$val = $(
							QWeb.render(
								"ks_db_list_preview_layout1",
								item_info,
							),
						);
						$val.find(".ks_dashboard_icon").css({
					...
					case "layout2":
						$val = $(
							QWeb.render(
								"ks_db_list_preview_layout2",
								item_info,
							),
						);
						var ks_rgba_dark_background_color_l2 =
							self._get_rgba_format(
								self.ks_get_dark_color(
									field.ks_background_color.split(",")[0],
									field.ks_background_color.split(",")[1],
									-10,
								),
							);
