odoo.define(
	"ks_dashboard_ninja_list.ks_dashboard_graph_preview",
	function (require) {
		"use strict";
		...
		var KsGraphPreview = AbstractField.extend({
			...
			start: function () {
				...
				core.bus.on("DOM_updated", this, function () {
					if (
						self.shouldRenderChart &&
						$.find("#ksMyChart").length > 0
					)
						self.renderChart();
				});
			...
			_render: function () {
				this.$el.empty();
				if (
					this.recordData.ks_dashboard_item_type !== "ks_tile" &&
					this.recordData.ks_dashboard_item_type !== "ks_kpi" &&
					this.recordData.ks_dashboard_item_type !== "ks_list_view"
				) {
					if (this.recordData.ks_model_id) {
						if (
							this.recordData.ks_chart_groupby_type ==
								"date_type" &&
							!this.recordData.ks_chart_date_groupby
						) {
							return this.$el.append(
								$("<div>").text(
									"Select Group by date to create chart based on date groupby",
								),
							);
						} else if (
							this.recordData.ks_chart_data_count_type ===
								"count" &&
							!this.recordData.ks_chart_relation_groupby
						) {
							this.$el.append(
								$("<div>").text(
									"Select Group By to create chart view",
								),
							);
						} else if (
							this.recordData.ks_chart_data_count_type !==
								"count" &&
							(this.recordData.ks_chart_measure_field.count ===
								0 ||
								!this.recordData.ks_chart_relation_groupby)
						) {
							this.$el.append(
			...
			_getChartData: function () {
				...
				this.chart_type =
					this.recordData.ks_dashboard_item_type.split("_")[1];
				...
				if (field.ks_chart_cumulative_field) {
					for (var i = 0; i < this.chart_data.datasets.length; i++) {
						...
						var data = [];
						if (
							this.chart_data.datasets[i]
								.ks_chart_cumulative_field
						) {
							for (
								var j = 0;
								j < this.chart_data.datasets[i].data.length;
								j++
							) {
								ks_temp_com =
									ks_temp_com +
									this.chart_data.datasets[i].data[j];
								data.push(ks_temp_com);
							}
							datasets.label =
								"Cumulative" +
								this.chart_data.datasets[i].label;
							datasets.data = data;
				...
				var $chartContainer = $(
					QWeb.render("ks_chart_form_view_container", {
				...
				if (this.chart_family === "circle") {
					if (
						this.chart_data &&
						this.chart_data["labels"].length > 30
					) {
						this.$el
							.find(".card-body")
							.empty()
			...
			renderChart: function () {
				var self = this;
				if (
					this.recordData.ks_chart_measure_field_2.count &&
					this.recordData.ks_dashboard_item_type === "ks_bar_chart"
				) {
					var self = this;
					var scales = {};
					scales.yAxes = [
						...
						},
						{
							...
							ticks: {
								...
								callback: function (value, index, values) {
									var ks_selection =
										self.chart_data.ks_selection;
									if (ks_selection === "monetary") {
										var ks_currency_id =
											self.chart_data.ks_currency;
										var ks_data =
											KsGlobalFunction._onKsGlobalFormatter(
												value,
												self.recordData.ks_data_format,
											);
				...
				if (this.chart_data && this.chart_data["datasets"].length > 0) {
					self.ksChartColors(
						this.recordData.ks_chart_item_color,
						this.ksMyChart,
						this.chart_type,
						this.chart_family,
						this.recordData.ks_show_data_value,
					);
				}
			},

			ksHideFunction: function (
				options,
				recordData,
				ksChartFamily,
				chartType,
				...
				if (ksChartFamily == "circle") {
					...
					options.tooltips.callbacks = {
						title: function (tooltipItem, data) {
							...
							if (ks_selection === "monetary") {
								var ks_currency_id =
									ks_self.chart_data.ks_currency;
					...
					if (
						this.recordData.ks_semi_circle_chart &&
					...
					options.plugins.datalabels.formatter = function (
						value,
						ctx,
					) {
						var ks_self = self;
						...
						if (ks_selection === "monetary") {
							...
							var ks_data = KsGlobalFunction._onKsGlobalFormatter(
								value,
								self.recordData.ks_data_format,
							);
							ks_data = KsGlobalFunction.ks_monetary(
