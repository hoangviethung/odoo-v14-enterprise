odoo.define(
	"ks_dashboard_ninja_list.ks_dashboard_ninja_list_view_preview",
	function (require) {
		"use strict";
		...
		var KsListViewPreview = AbstractField.extend({
				...
				if (rec.ks_dashboard_item_type === "ks_list_view") {
					if (rec.ks_list_view_type == "ungrouped") {
						if (rec.ks_list_view_fields.count !== 0) {
							...
							this.$el.append(
								$(
									'<div class="ks_nodata_list_view_preview">',
								).text("Select Fields to show in list view."),
							);
						}
					} else if (rec.ks_list_view_type == "grouped") {
						if (
							rec.ks_list_view_group_fields.count !== 0 &&
			...
			ksRenderListView: function () {
				...
				if (field.name) ks_list_view_name = field.name;
					...
					ks_list_view_name = field.ks_model_id.data.display_name;
					...
					if (index_data) {
						for (var i = 0; i < index_data.length; i++) {
							for (
								...
								if (date) {
									if (
										list_view_data.fields_type[index] ===
										"date"
									) {
										list_view_data.data_rows[j]["data"][
											index
				...
				if (field.ks_list_view_data) {
					...
					if (data_rows) {
						for (
							var i = 0;
							i < list_view_data.data_rows.length;
							i++
						) {
							for (
								var j = 0;
								...
								if (
									typeof list_view_data.data_rows[i].data[
										j
									] === "number" ||
				...
				count =
					list_view_data && field.ks_list_view_type === "ungrouped"
						? count - list_view_data.data_rows.length
						: false;
				count = count ? (count <= 0 ? false : count) : false;
				var $listViewContainer = $(
					QWeb.render("ks_list_view_container", {
