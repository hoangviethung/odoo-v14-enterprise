odoo.define("ks_dashboard_ninja_list.ks_color_picker", function (require) {
	...
	var KsColorPicker = AbstractField.extend({
		...
		jsLibs: ["/ks_dashboard_ninja/static/lib/js/spectrum.js"],
		cssLibs: ["/ks_dashboard_ninja/static/lib/css/spectrum.css"],
		...
		_render: function () {
			...
			var $view = $(
				QWeb.render("ks_color_picker_opacity_view", {
		...
		_ksOnColorChange: function (e, tinycolor) {
			this._setValue(
				tinycolor.toHexString().concat("," + this.value.split(",")[1]),
			);
		},
		...
		_ksOnOpacityInput: function (event) {
			...
			if (self.name == "ks_background_color") {
				color = $(".ks_db_item_preview_color_picker").css(
					"background-color",
				);
				$(".ks_db_item_preview_color_picker").css(
					"background-color",
					self.get_color_opacity_value(
						color,
				...
				color = $(".ks_dashboard_icon_color_picker > span").css(
					"color",
				);
				$(".ks_dashboard_icon_color_picker > span").css(
					"color",
					self.get_color_opacity_value(
						color,
						event.currentTarget.value,
